...
>В практическом использовании часто встречается конструкция с использованием лямбда-функций:
>
>```cpp
>screen.Post([...]() { ... })
>```
>
>Метод Post из FTXUI асинхронно выполняет лямбда‑выражение в основном потоке GUI. Это нужно, чтобы безопасно обновлять интерфейс из других потоков (например, из потока мониторинга USB‑портов). В квадратных скобкам мы указываем объекты, которые необходимо "захватить" в лямбду.
