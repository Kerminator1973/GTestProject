...
### Разработка собственных скритов поиска библиотек

Результатом поиска библиотеки является настройка переменных CMake. Как было описано ранее, поиск осуществляется с помощью скрипта `find_package(Foo)`. Мы можем разработать свой собственный скрипт поиска библиотеки. Для подключения с CMake, скрипт должен иметь имя соответствующее шаблоны "Find[имя_зависимости].cmake".

Для поиска библиотек часто используется утилита [pkg-config](./pkg-config.md).

В книге "Software Architecture with C++" by Adrian Ostrowski and Piotr Gaczkowski, в главе 7, приведён пример разработки такого скрипта.
