...
[Dependatbot](https://github.com/security/advanced-security) - инструмент, который автоматизирует обновление зависимостей в проекте и помогает следить за их безопасностью.

- Сканирует зависимости. Проверяет файлы манифестов (например, package.json, requirements.txt, Gemfile, pom.xml и т. д.) и смотрит, есть ли более новые или безопасные версии пакетов
- Создаёт pull request (PR). Если находит обновление, автоматически создаёт PR с изменением версии зависимости — чтобы вы могли его проверить и влить в код
- Реагирует на уязвимости. Если в какой‑то зависимости найдена уязвимость (CVE), Dependabot предложит версию, где она исправлена, и тоже откроет PR

Применяется на GitHub в автоматическом режиме.

Dependabot поддерживает экосистемы следующих языков программирования:

- JavaScript/Node.js: npm, yarn, pnpm
- Python: pip, poetry
- Java: Maven, Gradle
- .NET: NuGet
- Docker: образы в Dockerfile
- C++: vcpkg

Также существовали решения Renovate (www.mend.io) и Greenkeeper, которые также осуществляли контроль зависимостей. Однако, похоже, что они не пережили эпоху роста ИИ. Greenkeeper трансформировался во что-то называемое Snyk Code. Живой альтернативой этим решениям является [aikido.dev](https://www.aikido.dev/)
