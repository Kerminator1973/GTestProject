...
gRPC – удалённый вызов процедур, т.е обмен данными в парадигме «клиент-сервер». В приложениях ряде систем источником сообщений может быть как клиент, так и сервер, что потребует создания двух каналов с Protobuf, а не одного, как например, в случае использования WebSockets.

Поскольку технология gRPC является разработкой Google, то она, в первую очередь, ориентирована на использование технологического стека Google, то наилучшая поддержка обеспечивается для таких языков программирования как: Go, Python, Java, C++, JavaScript. Поддержка технологии для других языков программирования, которая предполагает наличие генераторов кода, значительно хуже. Так, в зоне риска оказываются такие языки, как: Rust, C#.

В браузерах нет встроенной поддержки Protobuf, в отличии, например, от WebSockets, который браузеры поддерживают "из коробки".

Добавление в проект (например, CMake) protobuf и gRPC может потребовать значительного количества времени, а загрузка и компиляция зависимостей может быть очень затратной по времени. Чтобы снизить затраты могут потребоваться значительные усилия на оптимизацию проекта и добавление специализированных функций-helper-ов.
