...
Рекомендуется создать в папке с проектом "CMakeExamples\BuildWithGoogleTest" подпапку с именем "build" и перейти в неё. Сделать это можно командами:
...
Находясь в папке build, можно запустить процесс поиска компиляторов и генерацию скрипта сборки для конкретной машины, следующей командой:

``` shell
cmake ..
```

Поскольку мы находимся внутри папки **build** две точки указывают, что сборку нужно начать в папке на уровень выше. Этот типовой подход позволяет упростить процесс удаления сгенерированных файлов тогда, когда они уже не нужны, или когда нужно создать "чистый стенд" для сборки исходников/тестирования скрипта сборки.
...
В данном случае, используется конфигурация **Debug**, но могут быть использованы и другие конфигурации, например, **Release**.

Если на Windows-машине установлена IDE Visual Studio, CMake сгенерирует соответствующий sln-файл. Мы можем верси разработку кода и выполнять его отладку прямо в Visual Studio.
...
>Clang произноситься как "клэнг", но некоторые разработчики предпочитают использовать "шланг".

>Следует заметить, что Clang совместим с gcc по параметрам командной строки.
